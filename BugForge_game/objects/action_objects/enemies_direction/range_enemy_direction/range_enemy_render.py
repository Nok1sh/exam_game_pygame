...
class RangeEnemyRender(pygame.sprite.Sprite):
    def __init__(self, enemy):
        super().__init__()
        self.range_enemy = enemy
        self.size_range_enemy: int = 22
        self.image = pygame.image.load(f"textures/range_enemy/range_enemy_1.png").convert_alpha()
        self.image = pygame.transform.scale(self.image, (
        self.image.get_width() // self.size_range_enemy, self.image.get_height() // self.size_range_enemy))
        self.rect = self.image.get_rect()
        self.rect.center = self.range_enemy.rect.center
